...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module: pl.LightningModule, batch: Any, batch_idx: int, split="train"):
        check_idx = (
            pl_module.current_epoch if self.logging_interval == "epoch" else pl_module.global_step
        )
        if (
            ...
            pl_module.logger.log_image(
                key=f"{split}/images", images=list(images.values()), caption=list(images.keys())
            )
    ...
    def on_validation_batch_end(
        ...
        if (
            ...
            and batch_idx == self.log_batch_idx[dataloader_idx]
        ):
